...
void f1(gsl_vector* x,gsl_vector* fx){ //(x+4)^2
    gsl_vector_set(fx,0,pow(gsl_vector_get(x,0)+4,2));        
}
...
void g3(gsl_vector* x, gsl_vector* fx){
    //rmax = 10 jf. opgaven
    e = gsl_vector_get(x, 0);
    double a = 1e-16, b = rmax, h = 1e-6, acc = 1e-5, ODEeps = 1e-4;
    gsl_vector* ya = gsl_vector_alloc(2);
...
int main(){
    ...
    printf("Finds roots of the function: f(x)=(x+4)^2 using Newton's method\n");
    ...
    printf("The resulting function are plotted in Hydrogen.pyxplot.png along with the exact function\n");

    //C
    printf("\n\nC\n");
    printf("Uses the boundary condition f(rmax)=0 to investigate convergence of the solution as a function of rmax\n");
    ...
    printf("Convergence shown in C1.pyxplot.png\n");
    
    printf("Uses the boundary condition f(rmax->infinity)= r*exp(-k*r) instead, where k = sqrt(-2ε)\n");
    gsl_vector* x5 = gsl_vector_alloc(1);
    FILE * file2 = fopen("C2.txt", "w"); 
    for(rmax = 1.5; rmax<15; rmax+=(double)1/2){
        gsl_vector_set(x5,0,-1);
        newton(g3,x5,eps);
        fprintf(file2,"%8.3g %.15g \n", rmax, gsl_vector_get(x5,0));
    }
    printf("Convergence shown in C2.pyxplot.png\n");
    printf("Compared in C1AndC2.pyxplot.png\n");
    printf("It is clear that C2 converges faster\n");
    fclose(file);
    fclose(file2);
}