...
int main(){
	// opgave A
    printf("A\n");  
    printf("\nFinds minimum of f(x)=(x+4)^2+7 using Quasi-newton\n");  //Quasi newton(qnewton) is coded in functions.c
	int n = 1;
	...
	qnewton(f1, x, eps);
	vector_print("x =", x);
    printf("Exact minimum x=-4\n");
    
    printf("\nFinds minimum of f(x,y) = x^4+3*y^2 using Quasi-newton\n"); 
	n = 2;
    ...
    printf("\nFinds minimum of Rosenbrock's valley function: f(x,y)=(1-x)^2+100*(y-x^2)^2 using Quasi-newton\n"); 
	n = 2;
    ...
    printf("\nFinds minimum of Himmelblau's valley function: f(x,y)=(x^2+y-11)^2+(x+y^2-7)^2 using Quasi-newton\n"); 
	n = 2;
    ...
    printf("Exact minimum both (3,2), (-2.805,3.131), (-3.779, -3.283) and (3.584, -1.848)\n"); 
    ...
    printf("\nThus, the mass of the boson is %.5g +- %.5g GeV/c^2\n", gsl_vector_get(x4,0), gsl_vector_get(x4,1)); 
	printf("Result from 2012: 125.3 +- 0.6 GeV/c^2\n"); 
    
	// Opgave C
    printf("\n\nC\n");
    printf("Finds minimum of the Rosenbrock function using downhill simplex\n"); 
	int d = 2;
	double* simplex[] = {
      (double[]){0.0, 0.0},
      (double[]){0.0, 5.0},
