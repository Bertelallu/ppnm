...
//finding the amount of points in input.dat
	int N=0;
...
//loading points from file(input.dat) into vectors x and y.
	double i;
	double j;
	int items;
	int n=0;
...
// GSL linear  and cubic interpolator.
...
//The values are written into a file linterp.dat. Along with the integral of that point.
