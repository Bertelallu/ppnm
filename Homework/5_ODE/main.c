...
void fNewton(double x, gsl_vector* y, gsl_vector* dydx){
    ...
    double R1 = pow(pow(y2-y0,2)+pow(y3-y1,2),0.5); //Længde^2 mellem r1 og r2
...
int main() {
    printf("A and B\n"); 
    printf("results are shown in Path_u_uprime.pyxplot.png\n"); 
    int n = 2; 
    ...
    printf("results are shown in PathSIR.pyxplot.png\n"); 

    n = 3; 
    gsl_vector* yaSIR = gsl_vector_alloc(n); 
    gsl_vector* ybSIR = gsl_vector_alloc(n); 
    gsl_vector_set(yaSIR,0,10000); 
    ...
    printf("results are shown in PathNewton.pyxplot.png and Figure8_Newton.pyxplot.png\n"); 
    ...
    a = 3, b = 20, h = 0.01, acc = 1e-3, eps = 1e-3;
