...
double f4(double x){calls++; return 2.0/(1.0 + 2*x*x);};

double gsl_f4(double x, void * params){calls++; return 2.0/(1.0 + 2*x*x);};
...
int main(){
	...
	printf("Integration of log(x)/sqrt(x) from %g to %g with eps = %g and accuracy = %g\n", a, b, eps, acc);
    printf("Calculated, Q = %.5g \n", Q1c);
    printf("Exact = %g\n", exact); 
    printf("Number of calls = %d\n", calls); 
    printf("Estimated error = %g\n", acc+fabs(Q1c)*eps); 
	...
	calls = 0;
	a = 0; b = 1;
	printf("Integration of log(x)/sqrt(x) from %g to %g with eps = %g and accuracy = %g using CC variable transformation\n", a, b, eps, acc);
    ...
    printf("Estimated error = %g\n", acc+fabs(Q2)*eps); 
    printf("Actual error = %g\n", fabs(Q2-exact)); 
    printf("Number of calls lowered by a factor of approximately 15 \n\n"); 
	...
	b = INFINITY;
    exact = 2.22144; 
    printf("Integration of 2.0/(1.0 + 2*x*x) from %g to %g with eps = %g and accuracy = %g\n", a, b, eps, acc);
